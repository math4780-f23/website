...
knitr::opts_chunk$set(
    ...
    out.width = "100%",
    fig.retina = 10,
    fig.path = "./images/14-colliearity/",
    message = FALSE,
    global.par = TRUE
)
options(
...
-   We are unable to choose the "best" combination of $\hat{\beta}_1$ and $\hat{\beta}_2$.
...
- After *unit length scaling*, ${\bf X'X} = \left[r_{ij}\right]_{k\times k}$ is the *correlation* matrix of $x$s denoted as ${\bf \Sigma}$. [^1] For example,
...
[^1]: The term *correlation* is a bit of misnomer. The regressors are not random variables. The correlation coefficient $r_{ij}$ does measure linear dependency between *$x_i$ and $x_j$ in the data*.
...
## [R Lab]{.pink} Hospital Manpower - Data
...
## Eigensystem Analysis: Variance Proportion
...
## [R Lab]{.pink} Hospital Manpower - Variance Proportion
```{r}
#| echo: true
...
## [R Lab]{.pink} Hospital Manpower - Ridge Regression
:::: {.columns}
...
Ainv <- solve(A)
var_ridge <- summary(lm_full_scale)$sigma^2 * Ainv %*% X_stX_s %*% Ainv
