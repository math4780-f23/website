...
## Statistics Comes In
Suppose each data point $Y_i$ of the sample $(Y_1, Y_2, \dots, Y_n)$ is a random variable from an identical population distribution $N(\mu, \sigma^2)$: $$Y_i \stackrel{iid}{\sim} N(\mu, \sigma^2), \quad i = 1, 2, \dots, n$$
...
$$\small P\left(\mu-t_{\alpha/2, n-1}\frac{S}{\sqrt{n}} < \overline{Y} < \mu + t_{\alpha/2, n-1}\frac{S}{\sqrt{n}} \right) = 1-\alpha$$
